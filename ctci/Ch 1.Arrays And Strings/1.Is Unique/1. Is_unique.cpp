...
// New function using hash table (unordered_set)
bool isUniqueChars_hashtable(const string &str) {
...
int main(){
        ...
        for (auto word : words)
        {
            cout << word << string(": ") << boolalpha << isUniqueChars_noDS(word) <<endl;  // Fixed: was calling bitvector function
        }
        cout <<endl << "Using hash table" <<endl;
        for (auto word : words)
        {
            cout << word << string(": ") << boolalpha << isUniqueChars_hashtable(word) <<endl;
        }
        return 0;
}